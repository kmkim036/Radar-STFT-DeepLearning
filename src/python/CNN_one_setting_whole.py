...
def preprocessing(person):  # person, motion에 해당하는 image 불러옴
    date = '220110'
...
def concatenate_n_div(image0, label0, image1, label1, image2, label2):
    ...
    x_train = np.concatenate((image0[0:int(
        count*train_ratio)], image1[0:int(count*train_ratio)], image2[0:int(count*train_ratio)]))
    y_train = np.concatenate((label0[0:int(
        count*train_ratio)], label1[0:int(count*train_ratio)], label2[0:int(count*train_ratio)]))
...
image0, label0 = preprocessing(0)  # 경민_motion 불러옴
image1, label1 = preprocessing(1)  # 성진_motion 불러옴
image2, label2 = preprocessing(2)  # 호정_motion 불러옴
...
for i in range(try_num):    # 5회 실행 -> 평균 계산위해
    ...
    image0_crop = preprocessing_resize_crop(
        image0_shuff, start_row, end_row, start_col, end_col, scale_row, scale_col)
    ...
    x_train, y_train, x_val, y_val, x_test, y_test = concatenate_n_div(
        image0_crop, label0, image1_crop, label1, image2_crop, label2)
