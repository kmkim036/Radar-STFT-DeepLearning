...
model = create_CNNmodel(lr, row_len, col_len, classnum)
early_stopping = EarlyStopping(monitor='val_accuracy', patience=10)
y_train = to_categorical(y_train, classnum)
y_val = to_categorical(y_val, classnum)
y_test = to_categorical(y_test, classnum)


hist = model.fit(x_train, y_train, validation_data=(
    x_val, y_val), epochs=10, callbacks=[early_stopping], verbose=2, batch_size=bs)
